...
import io.catalyte.training.sportsproducts.exceptions.UnprocessableData;
  ...
  private void handleLineItems(Purchase purchase) {
    ...
    if (itemsList != null) {
      itemsList.forEach(lineItem -> {
        ...
        if (product != null) {
          Boolean isProductActive = product.getActive();
          if (!isProductActive) {
            throw new UnprocessableData(product.getName() + " " + "is not active");
          }
